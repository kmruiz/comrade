...
## Note
**v0.4.0 exception, and the pre-tag checklist (2026-09-14).** The v0.4.0 tag points at commit 508c3e1 whose body describes the Anthropic preset and the authored-release-notes change — but a commit landed from a concurrent session *between* drafting the note and tagging (`21a056c feat(config): layer a project-level .comrade.toml over the user config`), so the release shipped a feature the body did not mention. The published body was corrected after the fact with `gh release edit v0.4.0 --notes-file`; for v0.4.0 the tagged commit body and the published body therefore differ. **Lesson: re-read `git log --oneline <prev-tag>..HEAD` immediately before writing the release-notes commit** — a commit landing concurrently is the one thing `release.sh` cannot detect (it only checks that HEAD is the release-notes commit and that its body is non-empty). Also: `.github/scripts/release-notes.sh` first appended the `**Full Changelog**` line unconditionally, so a body that already carried one came out with it twice; it now appends only when the description does not already contain one (commit e1d5a7a), which makes the duplication impossible.

## Note
**v0.4.0's release was created by hand (2026-09-14).** The tag push's workflow built all three binaries, but its `create GitHub release` step failed with `HTTP 403: Resource not accessible by integration` on `gh release create` — even though the identical command had succeeded for v0.3.0 at 10:07 and the workflow declares `permissions: contents: write`. Rerunning that single job then authenticated fine (it failed only because the release already existed), so the failure was transient, not a configuration fault. To finish the release I downloaded the run's artifacts (`gh run download 34837787573 -D /tmp/dist`) and ran `gh release create v0.4.0 --verify-tag --notes-file <corrected notes> <the three archives>`. Worth knowing: the repo's default workflow permission is `read` (`gh api repos/kmruiz/comrade/actions/permissions/workflow`), so if a create step 403s again, check that setting first.

## Note
**Workflow hardened for self-publishing (2026-09-14).** The transient 403 that forced the manual v0.4.0 publish is now handled in the workflow rather than by hand: the final step is "Create or update the GitHub release" and it retries `gh release create` up to 5 times with linear backoff, treats an already-published release as success by refreshing it (`gh release edit --notes-file` plus `gh release upload dist/* --clobber`), and passes `--target` only when the tag does not yet exist (on the tag path `gh` no longer touches the refs API). `set -uo pipefail` is deliberate (no `-e`, the script branches on exit codes). The `permissions: contents: write` block at the top of the workflow is what lets the job write releases and it is sufficient — the repository default is `read` (`gh api repos/kmruiz/comrade/actions/permissions/workflow`), but a workflow-level `permissions:` block overrides that default for its own jobs, and the workflow demonstrably created the v0.3.0 release with it. There is therefore no need to relax the repository default to `write`. Verified offline by extracting the step's `run` block and driving it with a stub `gh`: existing release -> edit+upload, exit 0; create failing once then succeeding -> exit 0; create failing always -> 5 attempts, exit 1.

## Note
**The release push deliberately bypasses the required `fmt + test` check (observed cutting v0.7.0, 2026-09-15).** `./release.sh` runs `git push origin main` immediately after `git push origin <tag>`, so at push time the `ci` job has not run on the new release commit yet and GitHub reports: `remote: Bypassed rule violations for refs/heads/main: Required status check "fmt + test" is expected.` The push still succeeds because the pushing account (the repo admin) has bypass permission. This is inherent to the flow, not a misconfiguration: the tag and the branch are pushed in the same command, so the gate cannot have completed. The consequence to remember: `main` can therefore advance on a release commit whose CI has not been verified, so the release branch's `ci` run is the only proof — check it before assuming the tagged tree is good. The practical mitigation is to run the same two commands locally before `./release.sh` (`cargo fmt --all -- --check` and `cargo test --workspace`), which is what was done for v0.7.0 (23 suites, 627 passed, 0 failed before tagging). If gating release commits on CI is ever wanted, the tag push must be split from the branch push (e.g. push the branch and let the workflow run, then tag).
