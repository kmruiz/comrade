...
## Note
2026-09-15 measured latency (release build, this machine). Added an ignored benchmark `bench_semantic` in crates/comrade-tool-memory/src/semantic/tests.rs (run: `cargo test -p comrade-tool-memory --release bench_semantic -- --ignored --nocapture`). Numbers on this repo (2337 code chunks over the tree, 115 memory docs): load code store from disk 14.6 µs (no cached index existed yet, so it rebuilt); model init (first embed) 231 ms; COLD code-index build (one-time walk+parse+embed of all 2337 chunks) 133.9 s (~57 ms/chunk — the dominant cost, paid once when the cache is missing or the tree changed a lot); memory index build 5.8 s (115 docs, ~50 ms/doc); WARM search (query embed + dot scan over both resident stores) 3.4–3.8 ms, stable across 5 repeats. So the steady-state query is sub-4 ms exactly as #22/#40 predicted; the linear scan is negligible and ANN/mmap remain unjustified. The remaining slow path is cold (re)build embedding throughput, not the query: embedding docs one long-text batch at a time on CPU costs ~50–57 ms each. In normal agent use the binary store is on disk, so cold start is store-load (µs) + model init (~0.2 s), and a dirty-but-unchanged tree re-parses without re-embedding or rewriting (content-based change detection).

## Note
2026-09-15 cold-path follow-up. The FIRST (cold) index build was reported as too slow (~2 min). Profiled with ignored benches in crates/comrade-tool-memory/src/semantic/tests.rs: `bench_chunk_stats` (chunk-length distribution + batching-padding simulation, no embedding), `bench_threads` (ORT intra-op thread scaling) and `bench_semantic` (end-to-end). Findings and fix:

COST = BATCH PADDING, not the scan or the store. This repo's ~2340 code chunks are wildly uneven (chars: p50=465, p90=1843, p99=4989, max=7792). fastembed's `transform` runs batches sequentially and ONNX pads every sequence in a batch to the longest one, so feeding chunks in file order made the batch-16 job pad 1.87M real chars to 5.76M — ~3x wasted compute (batch 32/64 worse still; batch 8 better).

FIX: `FastEmbedder::embed` now embeds the texts in ascending length order and maps the vectors back to the caller's order (`embed_ordered`). Zero quality change — same texts, only the processing order differs, and vector order is restored before it reaches the index. Measured cold code-index rebuild 133.9s -> 61.5s (2.2x), identical across two runs; memory index 5.8s -> 5.0s; warm query unchanged (~3.5ms). A regression test (`real_model_preserves_order_across_batches`) asserts each batched vector still best-matches its own text.

RULED OUT BY MEASUREMENT (not by guessing): (1) running K single-threaded sessions in parallel is *slower* than one session with one thread per core (4.1 vs 6.0-8.8 ms/text on 16 cores) — so a session pool was implemented, measured, and reverted; (2) a larger batch is slower (more padding); (3) ORT intra-op threading already scales (intra=1 4.36s -> intra=2 2.49s -> intra=4 1.74s -> default 1.47s for 128x~200-token texts) — sublinear past 4 threads is memory-bandwidth bound, i.e. no misconfiguration to fix.

TRIED AND REVERTED: capping the embedded chunk text at 1500 chars was a NO-OP (identical 61.51s) because the tokenizer already truncates those tails at the model's 512-token window, so the cap only removed text that was never embedded — a quality trade-off for ~0 gain. A content cap would need to go to <=~1000 chars (~250 tokens) to help; not chosen.

REMAINING cold cost is genuine CPU embedding (~15-17k tokens/s on this machine, ~26s of the 61.5s is the model across ~450k effective tokens; the rest is parse/tokenise). Further options, deliberately NOT taken: a content cap; a background warm-up of the index at project open; committing a prebuilt index.

## Note
semantic_search gained a `path` filter (GitHub feature request). The optional `path` arg restricts CODE hits to a file or directory; memory hits (ADR/glossary) have no file and are excluded when it is set. Matching is done by `path_matches(id, wanted)` in crates/comrade-tool-memory/src/semantic/mod.rs, which parses the file out of a `code:<rel>:<line>:<name>` id via `code_rel` and matches when the file is exactly the filter, lives under it (`dir/...`) or ends with it (`/name.rs`) after dropping a leading `./` and trailing `/` - so both `crates/comrade-tool-memory` and a bare `mod.rs` work. The filter is applied inside `rank` (new 5th parameter, alongside `kind`), so no separate index or re-embedding is needed.
