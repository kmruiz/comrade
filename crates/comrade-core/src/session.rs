...
pub enum AgentEvent {
    ...
    Thought(String),
    /// A tool call began. `tokens` is the real usage (prompt + completion) the
    /// model API reported for the request that produced this call, when
    /// available; for a turn that issued several calls it is attached to the
    /// first one only, so the UI can sum per-run usage without double counting.
    ToolCall {
        name: String,
        args: String,
        tokens: Option<usize>,
