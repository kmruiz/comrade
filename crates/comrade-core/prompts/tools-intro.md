...
- **Project facts** — dependencies, crates/subprojects, workspace layout, runnable tasks: call `pom_model` FIRST. Do NOT read the project manifest (e.g. Cargo.toml / package.json) just to answer such questions; pom_model already summarizes them.
...
- **Exact text** — a config key, an error string, a comment, or a name inside a string: `fs_rgrep`. It matches literal text (or a regex with `regex: true`) and is the FIRST choice for a literal search.
...
- **Only the meaning** — you know the domain but not yet the exact symbol, file or string ("where is session save/load handled?", "where do we retry?"): `semantic_search`. It searches ADR/glossary AND code by meaning and returns the closest hits with file:line. Reach for it EARLY to locate the relevant code and memory — it is the first choice whenever you know WHAT you want but not WHERE it lives, NOT a last resort. The tools above only help once you already hold a name.
