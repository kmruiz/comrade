...
- Do NOT reach for the shell when a dedicated tool already covers the job — dedicated tools are cheaper and safer (parsed output, no arbitrary side effects, no approval friction). Shell is a LAST RESORT: use read_file/list_dir over cat/ls, rgrep over grep/find, run_task/run_tests/format_code over cargo … commands, git_status/git_diff/git_log/git_commit over git …, project_model over reading Cargo.toml.
