...
2. Orient with the cheapest tool: read memory first (find_adr/find_glossary), then semantic_search EARLY to locate code+memory by meaning when you know the domain but not the exact symbol, file or string. Name-based tools come once you hold a name: pom_model for project facts, ts_find_symbol/ts_read_symbol for a symbol, fs_rgrep for exact text. Do not over-read.
